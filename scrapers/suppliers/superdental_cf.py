"""
Scraper for SuperDental (superdental.cl)
Platform: WooCommerce (Elementor frontend) — uses WC Store API
Cloudflare protected — uses cloudscraper
Products: Dental supplies, equipment, instruments
...
class SuperDentalCFScraper(BaseScraper):
    name = "SuperDental"
            ...
            try:
                ...
                for product in data:
                    try:
                        ...
                        logger.warning(f"[{self.name}] Error parsing product: {e}")
                        continue
